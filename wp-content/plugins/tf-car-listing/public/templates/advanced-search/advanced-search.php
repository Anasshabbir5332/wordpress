...
<div class="cards-container row <?php echo esc_attr( $map_position ); ?> <?php echo esc_attr( $archive_listing_sidebar == 'enable' ? 'has-sidebar' : 'no-sidebar' ); ?> <?php echo esc_attr( $archive_listing_sidebar_position ); ?>">
	<?php if ( $map_position == 'hide-map' || $map_position == 'map-header' ) : ?>
		<div class="<?php echo ( $archive_listing_sidebar == 'enable' ? 'col-xl-9 col-md-12' : 'col-md-12' ) ?> listing-list-wrap">
			<form method="get" action="<?php echo $query->have_posts() ? esc_url( get_page_link() ) : ''; ?>" class="tfcl-my-listing-search">
				<input type="hidden" name="layout_archive_listing" id="layout_archive_listing" value="<?php echo esc_attr( $layout_archive_listing ); ?>">
				<input type="hidden" name="column_layout" id="column_layout" value="<?php echo esc_attr( $layout_archive_listing == 'grid' ? $column_layout_grid : $column_layout_list ); ?>">
				<div class="row">
					<div class="col-lg-6">
				...
				<?php if ( $archive_listing_search_form_position == 'side' ) : ?>
					<div class="row group-filter-listing">
						...
						<div class="col-md-6 col-4 toolbar-search-list">
							<div class="wrap-reset-filter">
								<span class="btn-clear-filter"><i class="icon-autodeal-close2"></i> <span><?php esc_html_e( 'Clear Filters', 'tf-car-listing' ) ?></span></span>
							</div>
						<?php if ( tfcl_get_option( 'enable_switch_layout_button') == 'y') :?>
	...
	<?php if ( $map_position == 'half-map-right' || $map_position == 'half-map-left' ) : ?>
		<div class="col-lg-6 col-md-12 listing-list-wrap">
			<form method="get" action="<?php echo $query->have_posts() ? esc_url( get_page_link() ) : ''; ?>" class="tfcl-my-listing-search">
				...
				<div class="row">
					<div class="col-lg-6">
						<h2><?php esc_html_e( 'Listing', 'tf-car-listing' ); ?></h2>
					<?php if ( $total_post !== 0) :?>
						<p class="count-results">
							<?php echo sprintf( esc_html__('There are currently', 'tf-car-listing').' <span class="count-result count-total">%d</span> <span class="text-result">%s</span>', $total_post, tfcl_get_number_text( $total_post, esc_html__( 'results', 'tf-car-listing' ), esc_html__( 'result', 'tf-car-listing' ) ) ); ?>
						</p>
